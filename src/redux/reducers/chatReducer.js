    ...
    switch (action.type) {
      ...
      case "SOCKET_RECONNECTED":
        if (action.payload.connectionLostTime && state?.activeChat?.id !== -1) {
          const getData = async () => {
            if (cancelToken?.socket_reconnected)
              cancelToken?.socket_reconnected.cancel(
                "Operation cancel due to new request."
              );
            cancelToken.socket_reconnected = axios.CancelToken.source();
            const payload = {
              lastConnectedTime: action.payload.connectionLostTime,
              chatId: state.activeChat.id,
            };
            const config = {
              cancelToken: cancelToken?.socket_reconnected.token,
            };
            const data = await messageService.getLastMessage({
              payload,
              config,
            });
            if (data?.status) {
      ...
      case CHAT_CONST.SET_USER_NOTIFICATION:
        ...
        let newChatUsersFiltered = newChatList[
          NotificationIndex
        ]?.chatusers.find((x) => x.userId === action.userId);
        ...
        if (chatNotificationIndex !== -1 && newChatNotifyUserIndex !== -1) {
          newNotificationChatList[chatNotificationIndex].chatusers[
            ...
            isRoutineNotificationMute:
              action.payload?.isRoutineNotificationMute,
      ...
      case CHAT_CONST.RECEIVED_NEW_MESSAGE:
        ...
        if (
          action.payload?.messagerecipients &&
          !!action.payload?.messagerecipients.length
        ) {
          action.payload.messagerecipients =
            action.payload.messagerecipients.map((item) => {
      ...
      case CHAT_CONST.RECIVE_MESSAGE_UPDATE:
        ...
        if (!!newMessageList.length) {
          const MsgIndex = newMessageList.findIndex(
            ...
            return {
              ...
              messages: {
                data: {
                  ...
                  rows: action?.down
                    ? [
                        ...action.payload.data.rows.concat(
                          ...state.messages.data.rows
                        ),
                      ]
                    : [
                        ...state.messages.data.rows.concat(
      ...
      case CHAT_CONST.NEW_CHAT_RECEIVED:
        if (action.payload.id) {
          ...
          const alreadyAvailable = state.chatList.find(
      ...
      case CHAT_CONST.SET_USER_ONLINE:
        ...
        const userChatIndexOnline = newChatListOnline.findIndex(
          (item) =>
            item.users?.includes(action.payload.userId) &&
            item.type === CONST.CHAT_TYPE.PRIVATE
        );
      ...
      case CHAT_CONST.TOTAL_COUNT_DOWN:
        return {
          ...state,
          activeChat: { ...state.activeChat, totalCountDown: action.payload },
        };
      ...
      case CHAT_CONST.NEW_MESSAGE_COUNT:
        return {
          ...
          activeChat: {
            ...state.activeChat,
            newMessageCount:
              action.payload ?? (state.activeChat?.newMessageCount || 0) + 1,
          },
        };
      ...
      case "UPDATE_ISMESSAGE_GlobalSearch":
        const searchListIsMsg = state?.chatListData?.messages?.findIndex(
          (item) => item.id === action.payload.messageId
        );
          ...
          return {
            ...
            chatListData: {
              messages: state?.chatListData?.messages?.map((item, index) => {
                if (index === searchListIsMsg)
                  return { ...item, isMessage: action.payload.isMessage };
                return item;
              }),
            },
          };
        ...
        if (state.activeChat.id === action.payload.chatId) {
          if (action.payload.type === "remove-member")
            return {
              ...
              activeChat: {
                ...state.activeChat,
                chatusers: state.activeChat.chatusers.filter(
                  (i) => i.userId !== action.payload.userId
                ),
                users: state.activeChat.users.filter(
          ...
          if (action.payload.type === "add-members")
            return {
              ...
              activeChat: {
                ...state.activeChat,
                chatusers: state.activeChat.chatusers.concat(
                  action.payload.users
                ),
                users: state.activeChat.users.concat(
      ...
      case CHAT_CONST.UPDATE_EDITED_MESSAGE:
        ...
        const chatListdataEditMsg = state?.chatListData?.messages?.map(
          (item) => {
            if (item.id === action.payload.messageId)
              return { ...item, isEdited: true, ...action.payload.messageData };
            return item;
          }
        );
        if (indexEditMsg !== -1) {
      ...
      case CHAT_CONST.ADD_IMPORTANT_MESSAGE_SUCCESS:
        ...
        const updatedMessageList = messageList.map((item, index) => {
          if (item.id === action.payload.messageId)
            item.importantMessage = action.payload;
          return item;
        });
        const chatListMessageList = state?.chatListData?.messages?.map(
          (item, index) => {
            if (item.id === action.payload.messageId)
              item.importantMessage = action.payload;
            return item;
      ...
      case CHAT_CONST.REMOVE_IMPORTANT_MESSAGE_SUCCESS:
        ...
        const updatechatListMessageList = state?.chatListData?.messages?.map(
          (item, index) => {
            if (item.id === action.payload.messageId)
              item.importantMessage = null;
            return item;
      ...
      case CHAT_CONST.ADD_WATCH_MESSAGE_SUCCESS:
        ...
        const updatedWatchMessageList = messageWatchList.map((item, index) => {
          if (item.id === action.payload.messageId)
            item.task = { ...item.task, watchList: action.payload };
          return item;
        });
        const addWatchchatListMessageList = state?.chatListData?.messages?.map(
          (item, index) => {
            if (item.id === action.payload.messageId)
              item.task = { ...item.task, watchList: action.payload };
            return item;
      ...
      case CHAT_CONST.REMOVE_WATCH_MESSAGE_SUCCESS:
        ...
        const removeWatchUpdatedMessageList = removeWatchCurrentMessageList.map(
          (item, index) => {
            if (item.id === action.payload.messageId)
              item.task = { ...item.task, watchList: null };
            return item;
          }
        );
        const removeWatchchatListMessageList =
          state?.chatListData?.messages?.map((item, index) => {
            if (item.id === action.payload.messageId)
              item.task = { ...item.task, watchList: null };
            return item;
          });
      ...
      case CHAT_CONST.DELETE_GLOBAL_MESSAGE:
        ...
        return {
          ...
          chatListData: {
            messages: state.chatListData.messages.map((item, index) => {
              if (item.id === action.payload.deletedMessageId)
                return { ...item, ...updateObjBody };
              return item;
            }),
          },
        };
      ...
      case CHAT_CONST.UPDATE_SINGLE_CHAT_LIST:
        ...
        const dahsboardSingleUpdateIndex = UpdateSingleDashboardList.findIndex(
          (item) => item.id === action.payload?.id
        );
        if (chatSingleUpdateIndex !== -1 || dahsboardSingleUpdateIndex !== -1) {
          ...
          if (chatSingleUpdateIndex === -1) {
            const chatuserData = action.payload.chatusers.find(
              (i) => i.userId === action.userId
            );
            if (
              chatuserData &&
              ((!chatuserData.isEmergencyNotificationMute &&
                chatuserData.emergencyUnreadMessageCount) ||
                (!chatuserData.isUrgentNotificationMute &&
                  chatuserData.urgentUnreadMessageCount) ||
                (!chatuserData.isRoutineNotificationMute &&
                  chatuserData.routineUnreadMessageCount) ||
          ...
          return {
            ...state,
            chatList: makeUniqueObjects(
              UpdateSingleChatList.sort(compareDateTime)
            ),
            dashboardList: UpdateSingleDashboardList,
            hiddenChatNotiCount,
          };
          ...
          return {
            ...
            usersList: {
              ...
              users: newadminUsersList.map((item, index) => {
                if (index === usersUpdateIndex)
                  return { ...item, roleData: action.payload.roleData };
                return item;
              }),
            },
          };
      ...
      case CHAT_CONST.APPEND_SEARCH_CHAT_PATIENTLIST_DATA:
        if (action.payload) {
          ...
          if (
            action?.initial &&
            state.activePatientChat &&
            updatePatient[0].id === state.activePatientChat.id
          )
          ...
          return {
            ...
            patientList: makeUniqueObjects(
              action?.initial
                ? [...updatePatient, ...state.patientList]
                : state.patientList.concat(action.payload)
            ),
            // .sort((a, b) => {
            // 	let aTime, bTime;
      ...
      case CHAT_CONST.APPEND_SEARCH_CHAT_CATEGORYLIST_DATA:
        if (action.payload) {
          ...
          if (
            action?.initial &&
            state.activeCategoryChat &&
            updateCategory[0].id === state.activeCategoryChat.id
          )
      ...
      case CHAT_CONST.RES_CREATE_DESIGNATION:
        return {
          ...state,
          userDesignations: [action.payload, ...state.userDesignations].sort(
            compareByName
          ),
        };
      case CHAT_CONST.RES_DELETE_DESIGNATION:
        return {
          ...state,
          userDesignations: state.userDesignations
            .filter((item) => item.id !== parseInt(action.payload))
            .sort(compareByName),
        };
      ...
      case CHAT_CONST.VERIFY_HIDDEN_CHAT_MENTION:
        const counts = state.chatList.map((item) => {
          const chatuserData = item.chatusers.find(
            (i) => i.userId === action.userId
          );
          if (chatuserData && chatuserData.atTheRateMentionMessageCount) {
            if (
              ...
              return chatuserData.atTheRateMentionMessageCount;
          }
          return 0;
        });
        return {
          ...state,
          hiddenChatNotiCount: counts.reduce((a, b) => a + b, 0),
        };
