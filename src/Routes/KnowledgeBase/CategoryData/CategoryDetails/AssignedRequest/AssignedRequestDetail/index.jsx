...
import {
  DeleteIssuesAttachment,
  ReqUpdateIssue,
} from "redux/actions/IssuesAction";
...
export const AssignedRequestDetail = () => {
  const { assignIssueDetails: requestData } = useSelector(
    (state) => state.issues
  );
  ...
  const attchmentDeleteHandler = useCallback(
    async (id) => {
      try {
        await DeleteIssuesAttachment({ attachmentId: id });
        dispatch({
          type: ISSUE_CONST.RES_GET_REQUEST_DETAILS,
          payload: {
            ...requestData,
            issuesAttachments: requestData.issuesAttachments.filter(
              (att) => att.id !== id
            ),
          },
        });
  ...
  if (requestData) {
    try {
      ...
      const format_desc =
        ...
        sanitizeHTMLText(requestData.description, CONST.sanitize_message);
      return (
        <>
            ...
            <div className="request-subject">
              <div className={`${classes["new-issue-request"]} bg-card p-2`}>
                ...
                <div className="d-flex justify-content-between">
                  <div>
                    <span className="mr-2 fs-13">
                      Created By
                      <span className="light-text-70 ml-1">
                    ...
                    <span className="mx-2 fs-13">
                      Created
                      <span className="light-text-70 ml-1">
                        {moment(requestData.createdAt).fromNow()}
                      </span>
                    </span>
                    <span className="mx-2 fs-13">
                      Last Activity
                      <span className="light-text-70 ml-1">
                        {moment(requestData.updatedAt).fromNow()}
                      </span>
                    </span>
                  </div>
                  <div className="mx-1">
                    <div className="dropdown">
                      <button
                        className={`btn btn-info dropdown-toggle text-capitalize task-status-btn`}
                        id="labels"
                        type="button"
                      ...
                      <ul
                        className={`dropdown-menu dropdown-menu-right text-light m-0`}
                        aria-labelledby={`labels`}
                      >
                        {CONST.ISSUE_STATUS.map((item) => {
                          const status = requestData?.status;
                          if (
                            (status !== CONST.ISSUE_STATUS[0].value &&
                              item.value === CONST.ISSUE_STATUS[0].value) ||
                            (status === CONST.ISSUE_STATUS[0].value &&
                              item.value === CONST.ISSUE_STATUS[2].value) ||
                            (status === CONST.ISSUE_STATUS[1].value &&
                              item.value === CONST.ISSUE_STATUS[1].value) ||
                            item.value === status
                          )
                            return null;
                          return (
                ...
                {requestData.issuesAttachments &&
                  !!requestData.issuesAttachments.length && (
                    <>
