...
export default function WeekEvent({ event }) {
    ...
    const details = useMemo(() => {
        ...
        const format_subject = event.subject && messageFormat(event?.subject);
        const format_desc = event.description && messageFormat(event?.description);
        return (<>
            {format_subject &&
                <div className='font-weight-bold'>{format_subject}</div>}
            {format_patient1 &&
