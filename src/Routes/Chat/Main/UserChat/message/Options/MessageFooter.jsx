...
export default function MessageFooter({
  ...
  const setReactions = () =>
    dispatch({
      type: CHAT_CONST.SET_MESSAGE_REACTIONS,
      payload: item.messageEmojis,
    });
  let tagStatus = null;
  ...
  return (
    <div className="position-relative my-1">
      <div className="message-footer d-flex align-items-center justify-content-between flex-wrap">
        <div className={`${classes["status-block"]} m-0`}>
          {item.task?.dueDate && dueDateObj.visible && (
            <div className={`task-status ${dueDateObj.color} br-6`}>
              <p
                className="text-white text-capitalize px-1 mb-0"
                style={{ fontSize: user?.fontSize }}
              >
                {dueDateObj.isOverDue
                  ? "Overdue"
                  : `Due ${moment(item.task?.dueDate).format("MM/DD/YY")}`}
          ...
          {!item.isDeleted && item.type !== CONST.MSG_TYPE.ROUTINE && (
            <div
              ...
              className={`d-flex align-items-center ${
                classes["msg-task-status"]
              } ${getBackgroundColorClass("q-" + item.type)} px-1 h-auto`}
            >
          ...
          {!item?.isDeleted && item?.isMessage === false && tagStatus && (
            <div
              ...
              className={`d-flex align-items-center ${
                classes["msg-task-status"]
              } ${getTaskBackgroundColorClass(tagStatus)} px-1 h-auto mx-1`}
            >
              <span
          ...
          {!!Number(item?.replies) && (
            <div
              className={`d-flex align-items-center mx-1 text-link text-color`}
              onClick={setThreadMessage}
            >
              <span
        ...
        <div
          className="message-date text-capitalize message-info message-text d-flex"
          style={{ fontSize: user?.fontSize }}
        >
          {!item.isDeleted && (
            <>
                ...
                <>
                  {!!item?.task?.comments?.length && (
                    <MuiTooltip title="Task Comments">
                      <div
                        className={`d-flex align-items-center mx-1 ${classes["icon-space"]} board-task-card-svg`}
                      >
              ...
              {moveToOrigin && (
                <span
                  className="message-date text-capitalize mx-1"
                  role="button"
                  onClick={() => moveToOrigin(item)}
                >
                  <svg
                    height={user?.fontSize}
                    width={user?.fontSize}
                    fill="currentColor"
                  >
                    <path d="M7.375 14.375 3 10 7.375 5.625 8.438 6.688 5.875 9.25H14.5V7H16V10.75H5.875L8.438 13.312Z" />
                  </svg>
                </span>
              )}
              {item?.importantMessage && (
                <span className="message-star-icon">
                  <StarFill />
                </span>
              )}
              {item?.pinned && (
          ...
          <span className="mb-0">
            {moment(item.createdAt).format(
              withDate ? "MM/DD/YY hh:mm A" : "hh:mm A"
            )}
          </span>
          {item.sendBy === user.id && (
            <span className="ml-1">
              {isBufferMsg ? (
                ...
                <>
                  {item?.messageIsRead ? (
                    <CheckAll fill="#34b7f1" size={user?.fontSize + 2} />
                  ) : (
                    <Check size={user?.fontSize + 2} />
                  )}
                </>
              )}
            </span>
      ...
      {!hideReactions && !item.isDeleted && !!item?.messageEmojis?.length && (
        <div
          className="position-absolute reacted-div d-flex gap-5 px-1 emoji-font"
          onClick={setReactions}
        >
          {item.messageEmojis[0] && (
            <div className="" onClick={setReactions}>
              {item.messageEmojis[0].emojiCode}
            </div>
          )}
          {item.messageEmojis[1] && (
            <div className="" onClick={setReactions}>
              {item.messageEmojis[1].emojiCode}
            </div>
          )}
          {item.messageEmojis.length > 2 && (
            <div className="text-color" onClick={setReactions}>
              {item.messageEmojis.length}
            </div>
          )}
        </div>
      )}
      {item?.task?.watchList && <div className="keep-watch-task" />}
    </div>
  );
}
