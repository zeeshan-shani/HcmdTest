import React, {
  ...
  const PopupDataviewer = lazy(() =>
    import("Routes/Chat/Main/UserChat/message/Contents/PopupDataViewer")
  );
  ...
  export const ChatMessages = ({
    ...
    const userIndex = activeChat.chatusers.findIndex(
    ...
    const fetch = useCallback(
      async ({
        ...
        dispatch({
          type: CHAT_CONST.CLEAR_USER_NOTIFICATION,
          payload: { chatId: activeChat.id, userId: user.id },
        });
        if (unreadCount > CONST.MESSAGE_GET_LIMIT) {
          ...
          if (res.status === 1) {
            dispatch({
              type: CHAT_CONST.SET_OFFSET,
              payload: unreadCount - CONST.MESSAGE_GET_LIMIT,
            });
          ...
          if (res?.status === 1) {
            ...
            setTimeout(
              () =>
                ...
                messageRef[messageId]?.current?.scrollIntoView(false),
              200
            );
          }
        }
    ...
    const onClickNext = useCallback(async () => {
      if (!isLoading && !name) {
        isLoading = true;
        let messageId;
        if (!!messagesList.length) messageId = messagesList[0].id;
    ...
    const scrollToBottom = useCallback(async () => {
      await getMessages2({ pagitionFlow: "UP" }).then(async (data) => {
        if (data.status === 1)
    ...
    const ReqforQuotedMessage = useCallback(
      async (count, requestId) => {
        if (count > 100) {
          await getMessages2({
            messageId: requestId,
            pagitionFlow: "DOWN",
            ...
            if (data.status === 1) {
              dispatch({
                type: CHAT_CONST.TOTAL_COUNT_DOWN,
                payload: data.data.count - CONST.MESSAGE_GET_LIMIT,
              });
    ...
    const handlePinClick = (pMessage) => {
      pageScroll(messageRef[pMessage], { behavior: "smooth" });
      const classes = messageRef[pMessage].current.className;
      messageRef[pMessage].current.classList += " blink-quote-message ";
      setTimeout(() => {
        if (messageRef[pMessage])
          messageRef[pMessage].current.classList = classes;
      }, 2000);
    };
            ...
            await toastPromise({
              func: async (myResolve, myReject) => {
                try {
                  ...
                  if (res.data !== 0) {
                    await ReqforQuotedMessage(res.data, qMessage.id).then(
                      async (data) => {
                        if (data?.status) {
          ...
          if (Msg && Msg.hasOwnProperty("messagerecipients")) {
            if (Msg.messagerecipients?.isRead === false) {
              if (
                prevMsg === undefined ||
                (!isUnreadAvailable &&
                  CONST.MSG_TYPE.CHAT_LOG.TYPES.includes(prevMsg.type))
              ) {
    ...
    const onClickDiv = useCallback(() => {
      if (messageStatus)
        SetUserChatState((prev) => ({ ...prev, messageStatus: false }));
    ...
    const [newMsgGroupList, messagesIds, isTodayAvailable] = useMemo(() => {
      ...
      const groupMsgList = _.groupBy(messagesList, (item) =>
        moment(item.createdAt).format("MM/DD/YYYY")
      );
      const newMsgGroupList = Object.entries(groupMsgList).map(
        ([date, data]) => ({ date, data })
      );
      const messagesIds = messagesList
        .filter((i) => i.frontMsgId)
        .map((i) => i.frontMsgId);
      const isTodayAvailable = newMsgGroupList.some((i) =>
    ...
    const isGroupAdmin = useMemo(
      () =>
        activeChat.type === CONST.CHAT_TYPE.GROUP
          ? activeChat?.chatusers?.find((usr) => usr.userId === user.id)?.isAdmin
          : false,
      [activeChat, user.id]
    );
    ...
    try {
      ...
      return (
        <div
          className={`chat-content bg-chat-dark ${getChatBackgroudClass(
            user?.chatWallpaper
          )}`}
          id="messageBody"
          ref={chatContentRef}
        ...
        >
          ...
          <div className="container px-0" ref={ContainerRef}>
            ...
            >
              <div ref={messagesEndRef} className="mb-1">
                ...
                {state.bottomLoad ? (
                  ...
                  <hr className="chat-messages" />
                )}
                {/* <Button size="sm" onClick={() => onClickNext()}>Press to load more</Button> */}
              </div>
              {!!userChatState?.bufferMessages.length && (
                <div
                  className="message-day px-1"
                  id="messageDay"
                  key={-9}
                  name={moment().format("MM/DD/YY")}
                >
                  {!isTodayAvailable && (
                    <div
                      className="message-divider sticky-top my-1 p-1"
                      data-label={getDateLabel(moment().format("MM/DD/YY"))}
                  ...
                  <BufferMessages
                    messages={userChatState?.bufferMessages}
                ...
                {!!newMsgGroupList.length &&
                  newMsgGroupList?.map((groupItem, index_1) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className="message-divider sticky-top"
                        ...
                        <div className="d-flex flex-column-reverse">
                          {groupItem.data.map((item, index_2) => {
                            const broadcastContent =
                              activeChat.type === CONST.CHAT_TYPE.GROUP &&
                              item.type === CONST.MSG_TYPE.CHAT_LOG.NAME;
                            const index = counter;
                            counter++;
                            if (broadcastContent) {
                              return (
                                  ...
                                  {item?.pinned ? (
                                    ...
                                    >
                                      ...
                                      <Chip
                                        ...
                                        label={truncateText(item.message, 20)} // Adjust the maxLength as needed
                                      />
                                    </div>
                                  ) : (
                                    <></>
            ...
            <div
              className="scroll-bottom d-flex flex-column gap-5"
              style={{ zIndex: 9 }}
            >
              ...
              {activeChat.totalCountDown > 100 ? (
                <Badge
                  count={
                    activeChat.newMessageCount > 9
                      ? "9+"
                      : activeChat.newMessageCount
                  }
                  color="green"
                >
                  <MuiActionButton
          ...
          {taskDetails && taskName !== CHAT_MODELS.TODO && (
            <>
              <TaskDetails
                onClose={() => onCloseTaskDeatails(activeChat.id)}
                task={taskDetails}
              />
            </>
          )}
  ...
  export const ScrolltoOrigin = async (qMessage, recursiveCount = 2000) => {
    try {
      if (
        ...
        messageRef[qMessage.id].current.classList += " blink-quote-message ";
        setTimeout(() => {
          if (messageRef[qMessage.id]?.current)
            messageRef[qMessage.id].current.classList = classes;
        }, 3000);
      } else {
        setTimeout(
          () => recursiveCount && ScrolltoOrigin(qMessage, recursiveCount - 1),
          500
        );
      }
    } catch (error) {
